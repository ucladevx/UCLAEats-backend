...
def scrape_nutrition(soup):
    ...
    # p is children in nutrition box
...
def scraper_for_day_overview(date):
    """
    date is a string like 2018-04-21
    """
    overview_menu = {}
    menu_dict = {
        "menuDate": date.strip(),
    ...
    url = "http://menu.dining.ucla.edu/Menus/" + date
    soup = BeautifulSoup(requests.get(url).text,"lxml")
    ...
    for meal_header in soup.find_all("h2",id="page-header"):
        ...
        if len(meal_name_list) == 0:
            ...
            while get_next_sibling(menu_block_div).has_attr("class") and "menu-block" in get_next_sibling(menu_block_div)["class"]:
                ...
                overview_menu[meal_name][dining_hall_name] = {}
                
                # for each dining hall subsection
                ...
                for dining_hall_section_block in menu_block_div.find("ul",class_="sect-list").children:
                    if type(dining_hall_section_block) != element.NavigableString:
                        ...
                        overview_menu[meal_name][dining_hall_name][dining_hall_section_name] = []
                        for li_menu_item in dining_hall_section_block.find("ul",class_="item-list").children:
                            if type(li_menu_item) != element.NavigableString:
                                # get dish name
                                name = li_menu_item.find("a").string.strip()
                                # recipe name
                                ...
                                for img_icon in li_menu_item.find_all("img"):
                                    itemcoodes[img_icon["alt"]] = itemcode_dict[img_icon["alt"]];

                                # nutrition details
                                nutrition_soup = BeautifulSoup(requests.get(recipe_link).text,"lxml")
                                nutrition = scrape_nutrition(nutrition_soup)


                                overview_menu[meal_name][dining_hall_name][dining_hall_section_name].append({
                                    "name": name,
                                    "recipe_link": recipe_link,
                                    "itemcodes": itemcoodes,
                                    "nutrition": nutrition
                                })
...
def scraper_for_day_detail(date):
    """
    date is a string like 2018-04-21
    """
    detailed_menu = {}
    ...
    for meal in meal_list:
        url = "http://menu.dining.ucla.edu/Menus/" + date + "/" + meal
        soup = BeautifulSoup(requests.get(url).text,"lxml")
        ...
        for meal_header in soup.find_all("h2",id="page-header"):
            ...
            if len(meal_name_list) == 0:
                ...
                while get_next_sibling(menu_block_div).has_attr("class") and "menu-block" in get_next_sibling(menu_block_div)["class"]:
                    ...
                    dining_hall_name = menu_block_div.find("h3").string
                    print(dining_hall_name)
                    detailed_menu[meal_name][dining_hall_name] = {}
                    ...
                    for dining_hall_section_block in menu_block_div.find("ul",class_="sect-list").children:
                        if type(dining_hall_section_block) != element.NavigableString:
                            ...
                            detailed_menu[meal_name][dining_hall_name][dining_hall_section_name] = []
                            for li_menu_item in dining_hall_section_block.find("ul",class_="item-list").children:
                                if type(li_menu_item) != element.NavigableString:
                                    # get dish name
                                    name = li_menu_item.find("a").string.strip()
                                    ...
                                    detailed_menu[meal_name][dining_hall_name][dining_hall_section_name].append({
                                        "name": name,
                                        "recipe_link": recipe_link,
                                        "itemcodes": itemcoodes,
                                        "nutrition": nutrition
