...
function parseNutrition(nutrition_url, nutritions) {
    ...
    // do data cleaning
    Object.keys(nutritions).forEach(function(key){
        if(Array.isArray(nutritions[key])) {
            for(var i = 0; i < nutritions[key].length; i++) {
                if(nutritions[key][i][0] == '-' || nutritions[key][i] == null || nutritions[key][i] == undefined)
                    nutritions[key][i] = "-1";
            }
        }
        else {
